...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr readPointCloudKITTIWithRGB(
  std::string lidar_fileName, std::string img_fileName, Eigen::Matrix<float, 4, 4> calib_lidar,
  Eigen::Matrix<float, 4, 4> calib_camera)
{
  ...
  for (int nIndex = 0; nIndex < cloud->points.size(); nIndex++) {
    ...
    if ((p_u >= 0) && (p_u <= cols) && (p_v >= 0) && (p_v <= rows) && (p_w >= 0)) {
      cloud->points[nIndex].b = img.at<cv::Vec3b>(p_v, p_u)[0];  //not (p_u,p_v)!
...
pcl::PointCloud<pcl::PointXYZI>::Ptr fastMerge(
  std::vector<std::string> lidar_files, std::vector<Eigen::Matrix<float, 4, 4>> poses,
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr fastMergeWithImg(
  ...
  std::vector<Eigen::Matrix<float, 4, 4>> poses, Eigen::Matrix<float, 4, 4> calib_lidar,
  Eigen::Matrix<float, 4, 4> calib_camera)
{
  assert(poses.size() == lidar_files.size());
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr global_map =
    readPointCloudKITTIWithRGB(lidar_files[0], img_files[0], calib_lidar, calib_camera);
  // depthCrop(global_map, 50.0);
  pcl::transformPointCloud(*global_map, *global_map, calib_lidar);
  ...
  for (int i = 1; i < lidar_files.size(); i++) {
    ...
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_temp =
      readPointCloudKITTIWithRGB(lidar_files[i], img_files[i], calib_lidar, calib_camera);
    // depthCrop(cloud_temp, 50.0);
    Eigen::Matrix<float, 4, 4> trans = init_pose_inv * poses[i] * calib_lidar;
    pcl::transformPointCloud(*cloud_temp, *cloud_temp, trans);
    *global_map += *cloud_temp;
...
int main()
{
  std::string velodyne_path = "/home/yjq/dataset/KITTI_odometry/sequences/07/velodyne";
  std::string pose_path = "/home/yjq/dataset/KITTI_odometry/sequences/07/poses.txt";
  std::string calib_path = "/home/yjq/dataset/KITTI_odometry/sequences/07/calib.txt";
  std::string img_path = "/home/yjq/dataset/KITTI_odometry/sequences/07/image_3";
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr global_map =
    fastMergeWithImg(lidar_files, img_files, poses, calib_lidar, calib_camera);

  pcl::PointIndices::Ptr inliers(new pcl::PointIndices());
  pcl::ExtractIndices<pcl::PointXYZRGB> extract;
  for (int i = 0; i < global_map->size(); i++) {
    if (
      global_map->points[i].r == 0 && global_map->points[i].g == 0 &&
